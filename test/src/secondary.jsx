// Profile, Settings, Pricing, Admin views
function ProfileView() {
  const { Icon } = FMP;
  return (
    <div className="fade-in" style={{ padding: "28px 32px", maxWidth: 920, margin: "0 auto" }}>
      <h1 style={{ fontSize: 32, letterSpacing: "-0.025em", marginBottom: 4 }}>Your profile</h1>
      <p className="muted" style={{ marginBottom: 24 }}>What our matching engine knows about you. Edit any time.</p>

      <div className="card" style={{ padding: 24, marginBottom: 16 }}>
        <div className="row gap-4">
          <FMP.Avatar initials="AC" size="xl" tone="ink"/>
          <div style={{ flex: 1 }}>
            <h2 style={{ fontSize: 22 }}>Alex Chen</h2>
            <div className="muted">alex.chen@uw.edu · PhD applicant · Target Fall 2026</div>
            <div className="row gap-2 mt-3">
              <span className="pill">University of Washington</span>
              <span className="pill">BSc Computer Science</span>
              <span className="pill">English, Mandarin</span>
            </div>
          </div>
          <button className="btn"><Icon.Settings size={13}/> Edit</button>
        </div>
      </div>

      <div className="card" style={{ padding: 24, marginBottom: 16 }}>
        <div className="row between" style={{ marginBottom: 14 }}>
          <h3 style={{ fontSize: 15 }}>Research statement</h3>
          <button className="btn btn-sm btn-ghost">Re-embed</button>
        </div>
        <p style={{ fontFamily: '"Instrument Serif", serif', fontSize: 18, color: "var(--ink-2)", margin: 0, lineHeight: 1.5 }}>
          "I'm interested in multimodal large language models, especially how they reason about visual scenes through language. My undergrad thesis built a small vision–language model for chart understanding."
        </p>
        <div className="row between mt-4" style={{ fontSize: 11, color: "var(--ink-4)" }}>
          <span><Icon.Sparkles size={11}/> Last embedded 14 minutes ago · 384-dim vector</span>
          <span className="mono">sim with cohort median: 0.42</span>
        </div>
      </div>

      <div className="card" style={{ padding: 24, marginBottom: 16 }}>
        <h3 style={{ fontSize: 15, marginBottom: 14 }}>Match weights</h3>
        <p className="muted" style={{ fontSize: 12.5, marginBottom: 16 }}>
          We've learned your taste from 12 clicks and 3 sent emails. Suggested adjustments shown in green.
        </p>
        {[
          { k: "Expertise match",   v: 42, sug: 48, hint: "↑ You've clicked deeper into high-expertise matches" },
          { k: "Funding likelihood",v: 30, sug: 30 },
          { k: "Activity recency",  v: 18, sug: 14, hint: "↓ You opened older-paper profiles too" },
          { k: "Reputation",        v: 10, sug: 8 },
        ].map(it => (
          <div key={it.k} style={{ padding: "10px 0", borderTop: "1px solid var(--line)" }}>
            <div className="row between" style={{ marginBottom: 6 }}>
              <span style={{ fontSize: 13.5 }}>{it.k}</span>
              <div className="row gap-3" style={{ fontSize: 12 }}>
                <span className="mono">current {it.v}%</span>
                {it.sug !== it.v && <span className="mono pill pill-green">suggested {it.sug}%</span>}
              </div>
            </div>
            <div className="bar"><div className="bar-fill" style={{ width: it.v + "%" }}/></div>
            {it.hint && <div className="muted" style={{ fontSize: 11.5, marginTop: 6 }}>{it.hint}</div>}
          </div>
        ))}
        <button className="btn btn-primary btn-sm mt-4"><Icon.Sparkles size={12}/> Apply suggestions</button>
      </div>

      <div className="card" style={{ padding: 24 }}>
        <h3 style={{ fontSize: 15, marginBottom: 14 }}>Engagement history</h3>
...
function SettingsView() {
  ...
  return (
    <div className="fade-in" style={{ padding: "28px 32px", maxWidth: 920, margin: "0 auto" }}>
      <h1 style={{ fontSize: 32, letterSpacing: "-0.025em", marginBottom: 4 }}>Settings</h1>
      ...
      <SettingSection title="Account & billing">
        <Row label="Plan" right={<span className="pill pill-outline">Free · 5 emails/week</span>} hint="Upgrade for unlimited outreach, auto follow-ups, and CSV export."/>
        <Row label="Email signature" right={<button className="btn btn-sm">Edit</button>} hint="Appears at the bottom of every outreach email."/>
        <Row label="Sender identity" right={<span className="muted mono">alex.chen@uw.edu</span>} hint="DKIM/SPF verified · last checked 3h ago"/>
      </SettingSection>

      <SettingSection title="Data sources">
        {[
          ["ORCID", true, "synced 14m ago"],
          ["Semantic Scholar", true, "synced 14m ago"],
          ["arXiv", true, "synced 1h ago"],
          ["PubMed", false, "off"],
          ["NIH RePORTER", false, "off"],
          ["NSF Awards", true, "synced 6h ago"],
          ["CORDIS (EU grants)", true, "synced 6h ago"],
          ["LinkedIn", false, "disabled — ToS"],
        ].map(([n, on, s]) => (
          <Row key={n} label={n}
            right={<FMP.Avatar tone={on?"":"plain"} initials="" size="sm"/>}
            hint={s}
            customRight={
              <button onClick={() => {}} style={{
                width: 32, height: 18, borderRadius: 9999,
                background: on ? "var(--green-deep)" : "var(--paper-3)",
                border: 0, padding: 2, cursor: "pointer",
              }}>
                <span style={{ display: "block", width: 14, height: 14, background: "white", borderRadius: "50%", marginLeft: on ? 14 : 0, transition: "margin 0.15s" }}/>
              </button>
            }/>
        ))}
      </SettingSection>
      ...
      <SettingSection title="Notifications">
        <Row label="Weekly digest" hint="Top 5 new matches every Monday"
          customRight={<TogglePill on/>}/>
        <Row label="Reply notifications" hint="Instant push when a professor replies"
          customRight={<TogglePill on/>}/>
        <Row label="Funding alerts" hint="When a saved professor wins a new grant"
          customRight={<TogglePill on={false}/>}/>
        <Row label="Marketing emails" hint="Tips, blog posts, etc."
          customRight={<TogglePill on={false}/>}/>
      </SettingSection>

      <SettingSection title="Privacy & data">
        <Row label="Personalization" hint="Use my click & reply behavior to improve matching"
          customRight={<TogglePill on/>}/>
        <Row label="Analytics" hint="Anonymized usage signals" customRight={<TogglePill on/>}/>
        <Row label="Export all data" right={<button className="btn btn-sm"><Icon.ExternalLink size={12}/> Download</button>}/>
        <Row label="Delete account" right={<button className="btn btn-sm" style={{ color: "var(--red)", borderColor: "var(--red-soft)" }}>Delete</button>} hint="Right to be forgotten · GDPR"/>
      </SettingSection>
    </div>
  );
}
...
function Row({ label, hint, right, customRight }) {
  return (
    <div className="row between" style={{ padding: "14px 20px", borderTop: "1px solid var(--line)", borderTopColor: "transparent" }}>
      <div>
...
function TogglePill({ on }) {
  ...
  return (
    <button onClick={() => setV(!v)} style={{
      width: 32, height: 18, borderRadius: 9999,
      background: v ? "var(--green-deep)" : "var(--paper-3)",
      border: 0, padding: 2, cursor: "pointer",
    }}>
  ...
  return (
    <div className="fade-in" style={{ minHeight: "100vh", background: "var(--paper)" }}>
      <nav className="topnav" style={{ background: "transparent" }}>
        <div className="brand" onClick={onBack} style={{ cursor: "pointer" }}><Icon.Logo /> Find My Professor</div>
      ...
      <section style={{ padding: "64px 32px 24px", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: 52, letterSpacing: "-0.03em", maxWidth: 720, margin: "0 auto" }}>
          One coffee a week. <span style={{ fontFamily: '"Instrument Serif", serif', fontStyle: "italic", color: "var(--green-deep)" }}>One advisor for life.</span>
        </h1>
        ...
        <div className="row gap-1" style={{ justifyContent: "center", marginTop: 32, padding: 4, borderRadius: 999, background: "var(--paper-3)", display: "inline-flex" }}>
          {["monthly", "yearly"].map(b => (
            <button key={b} onClick={() => setBilling(b)} style={{
              ...
              {b[0].toUpperCase() + b.slice(1)} {b === "yearly" && <span style={{ color: "var(--green-deep)", fontSize: 11, marginLeft: 4 }}>−33%</span>}
            </button>
          ))}
        </div>
      </section>
            ...
            <div key={p.name} className="card" style={{
              ...
              <div className="row" style={{ alignItems: "baseline", gap: 4, marginBottom: 24 }}>
                {typeof p.price[billing] === "number" ? (
                  ...
                  <span style={{ fontSize: 44, letterSpacing: "-0.03em", fontWeight: 480, fontFamily: '"Instrument Serif", serif', fontStyle: "italic" }}>{p.price[billing]}</span>
                )}
              </div>
              <button className={"btn " + (p.highlighted ? "btn-primary" : "")} style={{ width: "100%", justifyContent: "center", padding: 12 }} onClick={onCta}>
                {p.cta}
              </button>
  ...
  return (
    <div className="fade-in" style={{ minHeight: "100vh", background: "var(--paper)" }}>
      <nav className="topnav">
        <div className="brand" onClick={onBack} style={{ cursor: "pointer" }}><Icon.Logo /> Find My Professor</div>
        <span className="pill pill-outline" style={{ marginLeft: 12 }}><Icon.Building size={11}/> Admin · University of Washington</span>
        <div style={{ marginLeft: "auto" }} className="row gap-2">
          <button className="btn btn-sm">Switch to student view</button>
          <FMP.Avatar initials="UW" tone="ink"/>
        </div>
      </nav>
      ...
      <div style={{ padding: "32px 32px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 16 }}>
          {[
            ...
            ["Open rate",       "58%", "+4pp vs cohort median"],
